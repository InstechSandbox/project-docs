...
- the shared runtime deploy now hard-fails after 45 minutes if a deploy run hangs, so one stuck apply cannot hold the deploy lane indefinitely
...
- the issuer backend task must point `TRUSTED_CAS_PATH`, `PRIVKEY_PATH`, `NONCE_KEY`, and `CREDENTIAL_KEY` at a writable runtime directory such as `/tmp/eudiw/pid-issuer-runtime/...`, not at ignored workspace-local files like `/app/local/*`
- the fixed Utopia DS signer material must be injected into the issuer backend through reviewed ECS secret references such as `UTOPIA_SIGNER_KEY_PEM`, `UTOPIA_SIGNER_CERT_PEM`, and `UTOPIA_SIGNER_CERT_DER_BASE64`; the container bootstrap then materializes `PID-DS-0001_UT` and `PID-DS-LOCAL-UT` filenames from those secrets before Flask imports the app
- the nonce key and credential-request encryption key can be generated ephemerally by container startup as long as the startup path also writes a matching metadata override JWK for `credential_request_encryption`
- the issuer backend must keep using the same credential-request key material after startup that it published in `credential_request_encryption`; do not reread or rotate `CREDENTIAL_KEY` independently from the generated metadata override during a running task
- the issuer frontend must not cache a stale copy of the backend `credential_request_encryption` JWK across backend task rotations; wallets fetch metadata from `issuer.<base-domain>` but post `/credential` to `issuer-api.<base-domain>`, so the frontend-hosted well-known metadata must refresh the backend request-encryption block before serving it or the wallet can encrypt to an old key and trigger `Failed to decrypt JWE ... InvalidTag()` on the backend
- the shared runtime deploy workflow now verifies after apply that `issuer.<base-domain>` and `issuer-api.<base-domain>` publish the same `credential_request_encryption` JWK before the deploy run is allowed to succeed
- the packaged Utopia SD-JWT signer certificate is local by default, so cloud issuer startup must rewrite the DS certificate SAN to the configured public `SERVICE_URL` before issuing JWT PID credentials; otherwise verifier SD-JWT validation rejects the posted proof because the embedded `x5c` leaf still advertises the local `192.168.x.x` issuer URI
