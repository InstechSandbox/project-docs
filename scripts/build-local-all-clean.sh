...
section "Wallet APK Clean Build"
(
  cd "$WALLET_REPO"
  LOCAL_DEMO_HOST="$PUBLIC_HOST" ./gradlew clean :app:assembleDemoDebug --console=plain
)

require_file "$APK_PATH"
ls -lh "$APK_PATH"

section "Verifier Docker Clean Build"
(
  cd "$VERIFIER_REPO"
